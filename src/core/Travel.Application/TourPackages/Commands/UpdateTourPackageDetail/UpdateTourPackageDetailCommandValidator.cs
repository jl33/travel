...
namespace Travel.Application.TourPackages.Commands.UpdateTourPackageDetail
{
    public class UpdateTourPackageDetailCommandValidator:AbstractValidator<UpdateTourPackageDetailCommand>
    {
        ...
        public UpdateTourPackageDetailCommandValidator(IApplicationDbContext context)
        {
            ...
            RuleFor(v => v.WhatToExpect).NotEmpty().WithMessage("WhatToExpect is required.");
            RuleFor(v => v.MapLocation).NotEmpty().WithMessage("MapLocation is required.");
            RuleFor(v => v.Price).NotEmpty().WithMessage("Price is required.");
            RuleFor(v => v.Duration).NotEmpty().WithMessage("Duration is required.");
            RuleFor(v => v.InstantConfirmation).NotEmpty().WithMessage("InstantConfirmation is required.");
